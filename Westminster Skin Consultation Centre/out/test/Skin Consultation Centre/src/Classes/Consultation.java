...
public class Consultation {
    ...
    public Consultation(Date date, int hours, Doctor doctor, Patient patient, String notes) {
        ...
        System.out.println(this.cost+" "+hours);
    ...
    public static Consultation findConsultationByAppointmentID(int appointmentID) {
        ...
        else return consultations.get(appointmentID - 1);
    }
        ...
        while(true){
            ...
            switch (i % 11) {
                ...
                case 2 -> {
                    ...
                    mobileNumber = ManagerOptions.readingNull(splinted[0].split(":")[1].trim());
                    dateOfBirth = formatter2.parse(ManagerOptions.readingNull(splinted[1].split(":")[1].trim()));
                }
                case 3 -> address = ManagerOptions.readingNull(line.split(":")[1].trim());
                case 4 -> note = line.split(":")[1].trim();
                case 5 -> date = formatter.parse(ManagerOptions.readingNull(line.split(":")[1].trim()));
                case 6 -> {
                    try {
                        hours = Integer.parseInt(line.split(":")[1].trim());
                    }catch (NumberFormatException ignored){}
                }
                case 7 -> medicalLicense = ManagerOptions.readingNull(line.split(":")[1].trim());
                case 8 -> {
                    try {
                        specialisationNum = Integer.parseInt(line.split(":")[1].trim());
                    }catch (NumberFormatException ignored){}
                }
                case 9 -> appointmentID = Integer.parseInt(line.split(":")[1].trim());
                case 0 -> {
                    ...
                    if (doctor!= null) {
                        ...
                        String message = "The doctor who owned medical license number = "+medicalLicense+", is no longer working in this institution.\n\n" +
                                " Under the said doctor, the reservation of the said person is canceled.\n\n Please call, "+mobileNumber+" and inform him.";
